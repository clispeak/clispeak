...
use clispeak_text::{Rejection, chunk, strip, validate};
...
fn a_word_that_merely_contains_www_is_not_a_url() {
    for text in ["Awww. Cute.", "swwwing", "Awww", "wwwww"] {
        assert!(
            clispeak_text::validate(text).is_ok(),
            "{text:?} is not a URL"
        );
    }
}
...
fn a_real_bare_url_is_still_refused() {
    for text in [
        ...
        assert!(
            clispeak_text::validate(text).is_err(),
...
fn a_number_is_not_split_across_chunks() {
    ...
    let chunks = clispeak_text::chunk(&long);
