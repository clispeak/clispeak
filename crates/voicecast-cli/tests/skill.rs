...
//!
//! `skills/voicecast/SKILL.md` is installed into agents so they can drive
...
/// The skill's text.
fn skill() -> String {
    // Three levels up from the crate: crates/voicecast-cli -> repo root.
    let path =
        std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("../../skills/voicecast/SKILL.md");
...
fn every_command_the_skill_names_exists() {
    ...
    for line in skill.lines() {
        for (at, _) in line.match_indices("voicecast ") {
            let rest = &line[at + "voicecast ".len()..];
    ...
    assert!(
        missing.is_empty(),
        "skills/voicecast/SKILL.md names commands that do not exist: {missing:?}\n\
...
fn every_flag_the_skill_names_exists() {
    ...
    assert!(
        missing.is_empty(),
        "skills/voicecast/SKILL.md names flags that do not exist: {missing:?}"
    );
}
