...
fn install_roots() -> Vec<PathBuf> {
    ...
    // `%LOCALAPPDATA%\clispeak`, which is also where the installer puts
    // Piper, and which needs no administrator to write.
    if let Some(dirs) = directories::BaseDirs::new() {
        roots.push(dirs.data_local_dir().join("clispeak"));
    }
    ...
    #[cfg(unix)]
    {
        roots.push(PathBuf::from("/app/share/clispeak"));
        roots.push(PathBuf::from("/usr/share/clispeak"));
    }
    roots
}
...
fn scratch_wav() -> PathBuf {
    ...
    std::env::temp_dir().join(format!("clispeak-{}-{n}.wav", std::process::id()))
}
...
const LOADER_ADVICE: &str = "is installed but will not start, because a library it needs is \
     ...
     which clispeak therefore installs beside it. Reinstalling clispeak restores that copy \
     — antivirus quarantine is the usual reason it goes missing.";
#[cfg(not(windows))]
const LOADER_ADVICE: &str = "is installed but will not start, because a library it needs is \
     missing. Reinstalling clispeak, or running `cargo xtask piper`, restores it.";
