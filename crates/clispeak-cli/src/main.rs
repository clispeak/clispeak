...
//! depends on `clispeak-proto` and `clispeak-text` but never on
//! `clispeak-core`, which is what keeps startup in single-digit
...
#[derive(Parser)]
#[command(name = "clispeak", about = "Speak text aloud on your devices", version)]
struct Cli {
...
enum Command {
    ...
    Replay {
        /// The message id, from `clispeak history`.
        msg_id: String,
    },
...
///
/// `println!` panics on EPIPE, so `clispeak ... | head -1` exited 101 —
...
async fn run() -> anyhow::Result<u8> {
    ...
    // `clispeak --to Phone mute` muted the machine it ran on, printed
...
acts on this device."
        ...
        err("");
        err(&format!(
            "It would have run here and said nothing about the device you named. Drop the flag:  clispeak {name}"
        ));
...
fn run_skill(install: bool, path: Option<&std::path::Path>) -> u8 {
    ...
    match skill::install(&destination) {
        Ok(()) => {
            ...
            err("Re-run this after upgrading clispeak to keep it in step.");
            exit::OK
        }
        Err(e) => {
            err(&format!(
...
fn list_groups(config: &config::Config) -> u8 {
    if config.groups.is_empty() {
        ...
        err("");
        err("Define one with:  clispeak group set phones pixel,iphone");
        return exit::OK;
    }
...
///
/// This used to say "no clispeak node is running" for every failure and send
/// the reader to `clispeakd`. That is a guess, and on macOS it is usually the
...
fn no_node(e: &std::io::Error) {
    ...
    if missing {
        err("error: nothing is listening for clispeak");
    } else {
        err(&format!(
            "error: the clispeak socket is not answering ({e})"
        ));
    }
    err("");
    ...
    err("");
    err("start one with: clispeakd, or open the clispeak app");
}
...
fn quiet_request(window: Option<&str>, high: bool, space: Option<String>) -> Result<Request, u8> {
    ...
    let Some((from, to)) = window.split_once('-') else {
        ...
        err("  clispeak quiet 22:00-07:00");
        err("  clispeak quiet off");
...
/// every later token — including real flags. `clispeak hello --to Phone` put
...
fn flags_first(argv: &[String]) -> Option<String> {
    ...
    Some(
        format!("clispeak {} {quoted}", flags.join(" "))
            .trim()
            .to_string(),
    )
}
...
fn build_speak(
    ...
    let text = if cli.raw {
        ...
        clispeak_text::strip(&text)
    } else {
        match clispeak_text::validate(&text) {
            Ok(()) => text,
            Err(rejection) => {
...
fn print_rejection(text: &str, rejection: &clispeak_text::Rejection) {
    ...
    // back (#65).
    let suggestion = clispeak_text::strip(text);
...
async fn send_with(request: Request, json: bool, unheard_only: bool) -> anyhow::Result<u8> {
    ...
    let token = match config::dir().as_deref().map(frame::read_token) {
        ...
        _ => {
            ...
            err("Open the clispeak app, or start clispeakd, then try again.");
            return Ok(exit::NO_NODE);
        }
    };
    if let Err(e) = frame::offer_handshake(&mut stream, &token).await {
        ...
        err(
            "Nothing was sent to it. Either it belongs to a different node — a \
             CLISPEAK_CONFIG_DIR that does not match CLISPEAK_SOCKET does this \
    ...
    Ok(match response {
        ...
        Response::Invite { url, expires_in } => {
            ...
            err("On the other device:  clispeak join <the line above>");
            exit::OK
        }
        Response::Preview {
            label,
            ...
            err("Join it with:  clispeak join <the same code>");
            exit::OK
        }
        ...
        Response::Joined { members, space } => {
            ...
            if space.starts_with("space-") {
                err(&format!(
                    "\nRename it with:  clispeak space rename {space} <name>"
                ));
            }
            exit::OK
        }
        ...
        Response::Rotated { space, devices } => {
            ...
            if devices.is_empty() {
                ...
                err("Run `clispeak invite` once per device.");
            }
            exit::OK
        }
...
fn exit_code_for(response: &Response, was_speak: bool) -> u8 {
    match response {
        Response::Error { kind, .. }
            if kind.as_deref() == Some(clispeak_proto::error_kind::NO_TARGET) =>
        {
            exit::NO_TARGET
        }
...
///
/// Sixteen, matching what `clispeak devices` prints, so a reader can compare
...
fn report(msg_id: &str, targets: &[clispeak_proto::TargetResult], json: bool) -> u8 {
...
mod display_tests {
    ...
    fn waiting_on_a_person_is_patient_and_everything_else_is_not() {
        use clispeak_proto::{Priority, Request};
        use std::time::Duration;
