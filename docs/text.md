...
```
$ tts --to pixel "Updated **3 files**. See \`src/main.rs\` for details."
...
`cat CHANGELOG.md | tts` becomes `cat CHANGELOG.md | tts --strip`. A changelog
...
The opposite case needs no special handling: `tts "build finished"` has no
